...
Sprite::Sprite(HWND hWnd, RECT clientRect)
{
	...
	// image = (HBITMAP)LoadImage(0, L"aurora.bmp", IMAGE_BITMAP, 0, 0, LR_LOADFROMFILE);
...
void Sprite::DrawRectangle(HWND hWnd)
{
	...
	// SelectObject(memDC, CreateSolidBrush(RGB(255, 192, 203)));
...
void Sprite::Draw(HWND hWnd)
{
	...
	{
		// properties.width = 200.0f;
		// properties.height = 200.0f;
		MakeABounce();
		DrawRectangle(hWnd);
	}
}
